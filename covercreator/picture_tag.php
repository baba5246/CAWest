	...
	if ($fb_user){
		if(($template_id)){
			...
			foreach($tag_photos_query_result["data"] as $data){
				...
				//tagの最終日付は配列要素の最初["tags"]["data"][0]
				$photos_list[$cnt]["tag_updated_time"] = strtotime($data["tags"]["data"][0]["created_time"]);
		...
		if($action_flg=="photos_select"){
				if($photo_place == 1){
					...
					$ret = imagejpeg($screen_new,$LOG_PICTURE."tags/".$fb_user."/photo_".$fb_user."_".$photos_id."_".$photo_place.".jpeg");
...
//console.log(cover_position_top)
...
<?php if(($template_id)&&($action_flg=="photos_select")){
			...
			$size_photo_source = getimagesize($LOG_PICTURE."tags/".$fb_user."/display_photo_".$fb_user."_".$photos_id."_".$photo_place.".jpeg");

?>

		<div id="scroll" style="width:770px;height:285px;overflow:hidden;cursor:move;position:relative;">
		<?php if($device_code == "pc"){?>
		<img id="draggable1" src="<?php echo($LOG_PICTURE."tags/".$fb_user."/display_photo_".$fb_user."_".$photos_id."_".$photo_place.".jpeg");?>" alt="" width="<?php echo($size_photo_source[0]);?>px" height="<?php echo($size_photo_source[1]);?>px"/><div id="container" ><h2>カバーをドラックして、再配置</h2></div>
		<?php }else{?>
...
<div id="page_btn">
<div id="btn_back"><a href="template_common.php?category_id=tag"><img src="img/btn_back.jpg" width="275" height="61" alt="BACK"></a>
</div>
<!--
<div id="btn_next"><a href="#"><img src="img/btn_next.jpg" width="275" height="61" alt="NEXT" onClick="javascript:dummy()"></a>
-->
<div id="btn_next"><img src="img/btn_next.jpg" width="275" height="61" alt="NEXT" onClick="javascript:doPhotosSelect('check_tag','check_tag','<?php echo($template_id);?>','<?php echo($photos_id);?>','<?php echo($photo_place);?>','<?php echo($photo_source);?>','<?php echo($page_id);?>')">
</div>
</div><!--id="page_btn"-->
...
<input type="hidden" name="page_no_photo" value="" />
...
$endTime = microtime(true);
	...
	$my365_user_id = "";
	$insta_user_id = "";
	$access_page = "picture_tag_".$action_flg."_".$template_id;
